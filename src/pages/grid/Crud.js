...
import { APP } from "src/scripts/env";
...
const crud = {
  ...
  path(path, body) {
    let query = "";
    if (util.isDefined(body.stateless)) {
      query += "stateless=" + body.stateless + "&";
    }
    if (util.isDefined(body.showInfo)) {
      query += "showInfo=" + body.showInfo + "&";
    }
    if (util.isDefined(body.showAllFields)) {
      query += "showAllFields=" + body.showAllFields + "&";
    }
    if (util.isDefined(body.useNative)) {
      query += "useNative=" + body.useNative + "&";
    }
    if (query.length != 0) {
      query =
        (path.indexOf("?") !== -1 ? "&" : "?") +
  ...
  action: {
    ...
    delete(input) {
      ...
      uix.confirm(function () {
        ...
        let path = crud.path("/crud/delete", body);
        if (ids.length === 1) {
          body.id = ids[0];
        } else {
          body.ids = ids;
      ...
      api.call({
        ...
        onSuccess(data) {
          ...
          if (refresh) {
            ...
            api.call({
              path: crud.path("/crud/single", body),
              method: "post",
              data: body,
              onFinish() {
                util.runIf(util.isFunction(input.onProgress), () =>
