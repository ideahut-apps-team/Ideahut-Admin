...
export default {
  ...
  created() {
    ...
    self.title = crud.isString(params.title) ? params.title : template.title;
    self.replica = crud.get.number(params.replica, null);
    self.fields = [];
    let cols = crud.get.array(template.table.columns);
    let value;
    let isIdShown = false;
    ...
    if (!isIdShown) {
      let value = crud.id.toPk(template.id, row);
    ...
    self.forms = crud.get.array(template.forms);
    self.tables = crud.get.array(template.children);
    ...
    on_form_click(form, index) {
      ...
      crud.api.call({
        ...
        onSuccess(data) {
          ...
          crud.inject.pkAndGridId(form.id, data, template._grid_id_);
