...
const util = {
  ...
  format: {
    ...
    money(number, options) {
      let opt = util.isObject(options) ? options : {};
      let places = Math.abs(opt.places);
      places = !isNaN(places) ? places : 2;
      let symbol = util.isString(opt.symbol) ? opt.symbol : "";
      let thousand = util.isString(opt.thousand) ? opt.thousand : ".";
      let decimal = util.isString(opt.decimal) ? opt.decimal : ",";
  ...
  sort: {
    array(array, field, opts = { ascending: true }) {
      if (util.isArray(array) && util.isString(field)) {
        array.sort((a, b) => {
          const la =
            true === opts.sensitive ? a[field].toUpperCase() : a[field];
